...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
    ...
    elif query.data.startswith("pm_setgs"):
        ...
        btn = [[
            InlineKeyboardButton("Go To The Chat", url=f"https://t.me/{temp.U_NAME}")
        ]]
        ...
        if settings is not None:
            ...
            try:
                ...
                return await query.answer('You blocked me, Unblock me and try again...', show_alert=True)

    elif query.data.startswith("grp_setgs"):
        ident, grp_id = query.data.split("#")
        userid = query.from_user.id if query.from_user else None
        st = await client.get_chat_member(grp_id, userid)
        if (
...
async def advantage_spell_chok(msg):
    ...
    gs_parsed = []
    if not g_s:
        btn = [[InlineKeyboardButton("🔎 Search Google 🔍", url=f"https://www.google.com/search?q={search}")]]
